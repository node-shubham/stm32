...
int main()
{
	...
	while(1)
	{
		...
		i2c_senddata(&i2c1_handle, &cmd_len, 1, SLAVE_ADDR, I2C_ENABLE_RS);

		i2c_receivedata(&i2c1_handle, &len, 1, SLAVE_ADDR, I2C_ENABLE_RS);

		i2c_senddata(&i2c1_handle, &cmd_data, 1, SLAVE_ADDR, I2C_ENABLE_RS);
		i2c_receivedata(&i2c1_handle, data, len, SLAVE_ADDR, I2C_DISABLE_RS);
		data[len+1]= '\0';
		printf("data = %s\n",data);
